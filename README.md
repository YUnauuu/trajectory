...
## 2024/3/19
1. 剪枝策略：有轨迹P，Q 如果轨迹P 中的点到轨迹Q中点Qk的距离大于阈值（阈值为MAX{[(Q1,Q2)+(Q1,Q2)+...+(Qk-1,Qk)],[(Qk,Qk+1),(Qk+1,Qk+2)+..+(Qn-1,Qn)]}）则不再匹配Q中的点，直接用这个距离。如果小于阈值，则继续匹配Q中的点，并把Q均分成两部分，对每一个部分重新设置阈值重复以上操作。
